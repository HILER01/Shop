...
public ShopAction OnEquipItem(int client, CategoryId category_id, const char[] category, ItemId item_id, const char[] cItem, bool isOn, bool elapsed)
{
	...
	PrintToChat(client, "Не удалось активировать \"%s\" неон.", cItem);
	return Shop_Raw;
}
...
public Action Event_OnPlayerSpawn(Handle event, const char[] name, bool silent)
{
...
public Action Event_OnPlayerDeath(Handle event, const char[] name, bool silent)
{
	RemoveClientNeon(GetClientOfUserId(GetEventInt(event, "userid")), false);
}

void RemoveClientNeon(int client, bool reset = true)
{
